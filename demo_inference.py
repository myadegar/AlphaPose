...
parser.add_argument('--cfg', type=str, required=False,
                    help='experiment configure file name', default='configs/halpe_26/resnet/256x192_res50_lr1e-3_1x.yaml')
parser.add_argument('--checkpoint', type=str, required=False,
                    help='checkpoint file name', default='pretrained_models/halpe26_fast_res50_256x192.pth')
...
parser.add_argument('--indir', dest='inputpath',
                    help='image-directory', default="result/demo/")
...
parser.add_argument('--outdir_plot', dest='outputpath_plot',
                    help='output-directory of plots', default="result/plot")
...
parser.add_argument('--video', dest='video',
                    help='video-name', default="new_example/Exercise Videos 1st10/t11.mp4")
...
if __name__ == "__main__":
    ...
    if not os.path.exists(args.outputpath_plot):
        os.makedirs(args.outputpath_plot)
    ...
    try:
        for i in im_names_desc:
            ...
            with torch.no_grad():
                ...
                # select one human (maximum detection score)
                if scores is not None:
                    idx_max = torch.argmax(scores)
                    scores = torch.index_select(scores, 0, idx_max)
                    inps = torch.index_select(inps, 0, idx_max)
                    boxes = torch.index_select(boxes, 0, idx_max)
                    ids = torch.index_select(ids, 0, idx_max)
                    cropped_boxes = torch.index_select(cropped_boxes, 0, idx_max)
